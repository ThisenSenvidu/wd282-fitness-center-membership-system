...
public class TrainerService {
    ...
    public ArrayList<Trainer> getAllTrainers() {
        try { return fileHandler.getAllTrainers(); }
        catch (Exception e) { throw new RuntimeException("Could not load trainers: " + e.getMessage()); }
    }

    public Trainer getTrainerById(String id) {
        try { return fileHandler.getTrainerById(id); }
        catch (Exception e) { throw new RuntimeException("Could not find trainer: " + e.getMessage()); }
    }

    public ArrayList<Trainer> searchTrainers(String keyword) {
        try { return fileHandler.searchTrainers(keyword); }
        catch (Exception e) { throw new RuntimeException("Search failed: " + e.getMessage()); }
    }

    // Polymorphism: picks the right subclass based on trainerType
    public void addTrainer(String name, String email, String phone,
                           String specialization, String availability, String trainerType) {
        Trainer trainer;
        ...
        try { fileHandler.addTrainer(trainer); }
        catch (Exception e) { throw new RuntimeException("Could not save trainer: " + e.getMessage()); }
    }

    public boolean updateTrainer(String trainerId, String name, String email, String phone,
                                 String specialization, String availability, String trainerType) {
        Trainer updated;
        if ("personal".equals(trainerType)) {
        ...
        try { return fileHandler.updateTrainer(updated); }
        catch (Exception e) { throw new RuntimeException("Could not update trainer: " + e.getMessage()); }
    }

    public boolean deleteTrainer(String id) {
        try { return fileHandler.deleteTrainer(id); }
        catch (Exception e) { throw new RuntimeException("Could not delete trainer: " + e.getMessage()); }
    }
}
